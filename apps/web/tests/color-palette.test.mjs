...
test('custom palettes support every section without depending on a palette name',()=>{
 ...
 for(const section of PALETTE_SECTIONS) palette[section]={backgroundColor:'#113322',textColor:'#FFFFFF',accentColor:'#AA9900'};
...
test('gradients preserve stops and valid angles while rejecting CSS injection',()=>{
 const result=paletteStyle({founder:{backgroundColor:'#F6F1E7',backgroundMiddleColor:'#E7D3A6',backgroundEndColor:'#F6F1E7',gradientAngle:120},news:{backgroundColor:'url(https://example.com)',textColor:'#ffffff;display:none'},header:{backgroundColor:'#24303D',backgroundEndColor:'#111820',gradientAngle:Infinity}});
 assert.equal(result['--palette-founder-background'],'linear-gradient(120deg, #F6F1E7, #E7D3A6, #F6F1E7)');
...
test('published and draft Site Settings load palette colors through the API',async()=>{
 const queries=[];
 const {getSiteSettings}=loadTs('../src/lib/site-settings.ts',{env:{STRAPI_URL:'http://cms.test',STRAPI_API_TOKEN:'test'},fetch:async(_url,init)=>{
  queries.push(JSON.parse(init.body).query);
  return {ok:true,json:async()=>({data:{siteSetting:{activePalette:{name:'Secondary',founder:{backgroundColor:'#FFFFFF'}}}}})};
 }});
...
test('palette background overrides old section colors; testimonial fade follows the palette',()=>{
 ...
 const tests=readFileSync(new URL('../src/components/testimonials.css',import.meta.url),'utf8');
...
test('fresh palette reads see consecutive selections even in production', async()=>{
 ...
 const {getFreshPalette}=loadTs('../src/lib/active-palette.ts',{env:{NODE_ENV:'production',STRAPI_URL:'http://cms.test',STRAPI_API_TOKEN:'test'},fetch:async()=>({ok:true,json:async()=>({data:{siteSetting:{activePalette:{name:selected,founder:{backgroundColor:selected==='Primary'?'#F6F1E7':'#FFFFFF'}}}}})})});
...
test('switching palettes removes stale gradient variables without replacing unrelated body styles',()=>{
 ...
 const values=new Map([['--palette-founder-background','linear-gradient(120deg, #F6F1E7, #E7D3A6)'],['--palette-founder-background-end-color','#E7D3A6'],['overflow','hidden'],['--font-display','Georgia']]);
 const body={dataset:{siteMode:'primary',colorPalette:'Primary'},style:{[Symbol.iterator]:()=>values.keys(),removeProperty:key=>values.delete(key),setProperty:(key,value)=>values.set(key,value)}};
...
test('new Secondary palettes include the local design’s black gallery',async()=>{
 ...
 const created=[];
 const documents={findFirst:async()=>null,create:async({data})=>{created.push(data);return {...data,documentId:data.name};}};
