...
test("loads every published comment beyond the first hundred in order", async () => {
  const requests = [];
  const api = loadTs("../src/lib/comments.ts", {
    env,
    fetch: async (_url, options) => {
      const request = JSON.parse(options.body);
      requests.push(request);
      assert.match(request.query, /status:\s*PUBLISHED/);
      ...
      return {
        ...
        json: async () => ({
          data: {
            comments_connection: {
              nodes: Array.from({ length: page === 3 ? 5 : 100 }, (_, i) => ({
                documentId: String((page - 1) * 100 + i),
              })),
  ...
  assert.equal(items[204].documentId, "204");
  assert.deepEqual(
    requests.map((r) => r.variables.page),
    [1, 2, 3],
  );
});
test("empty published collection stays empty", async () => {
  const api = loadTs("../src/lib/comments.ts", {
    ...
    fetch: async () => ({
      ok: true,
      json: async () => ({
        data: { comments_connection: { nodes: [], pageInfo: { pageCount: 0 } } },
      }),
    }),
  });
