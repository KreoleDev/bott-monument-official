...
export function GalleryCarousel({ section, items }: { section: HomepageSection; items: (GalleryItem & { src: string })[] }) {
  ...
  function closeAccess() { dialog.current?.close(); restoreScroll(); }
  const requestHref = access?.requestHref && /^(#[\w-]+|\/(?!\/))/.test(access.requestHref) ? access.requestHref : '#contact';

  return <section id="magazine" aria-labelledby="gallery-title" style={{ '--gallery-background': section.backgroundColor || undefined, '--gallery-text': section.textColor || undefined } as CSSProperties}>
    <div className="gallery-variant" id="magCircularVariant">
      <div className="cg-header-shell">
        <div className="cg-header">
          <h2 className="cg-title" id="gallery-title">{section.title === 'A Gallery of Lasting Tributes' ? <>A Gallery of Lasting<em>Tributes</em></> : section.title}</h2>
        </div>
        {access && <button className="mag-access-trigger cg-access-trigger" type="button" aria-haspopup="dialog" aria-controls="magAccessModal" onClick={openAccess}>
          <span className="mag-access-trigger-text">{section.buttonLabel}</span>
        </button>}
      </div>
      <p id="gallery-instructions" className="sr-only">Drag left or right to rotate. Hover over a card to pause and enlarge it. With the gallery focused, use the arrow keys to rotate and Space to pause or resume.</p>
      <div ref={viewport} className="cg-viewport" role="region" aria-roledescription="carousel" aria-label="Rotating memorial gallery" aria-describedby="gallery-instructions" tabIndex={0}>
    ...
    {access && <dialog ref={dialog} id="magAccessModal" className="mag-access-modal" aria-labelledby="magAccessTitle" aria-describedby="magAccessCopy" onClose={restoreScroll} onClick={e => { if (e.target === e.currentTarget) closeAccess(); }}>
      <div className="mag-access-card">
        ...
        <h3 className="mag-access-title" id="magAccessTitle">{access.title} <em>{access.emphasis}</em></h3>
        <p className="mag-access-copy" id="magAccessCopy">{access.description}</p>
        <div className="mag-access-actions">
          <a className="mag-access-primary" href={requestHref} onClick={closeAccess}>{access.requestLabel}</a>
          <button className="mag-access-secondary" type="button" onClick={closeAccess}>{access.dismissLabel}</button>
        </div>
      </div>
    </dialog>}
  </section>;
