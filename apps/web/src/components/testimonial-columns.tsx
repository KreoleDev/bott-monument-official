...
function Column({ comments, index }: { comments: Comment[]; index: number }) {
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      current = target = reduced.matches ? 0 : height * (0.14 + index * 0.13);
    };
    ...
    const tick = (time: number) => {
      ...
      last = time;
      if (!reduced.matches && !hovering && !focused && !paused && drag === null)
        target = mod(target + [18, 15, 12][index] * dt);
      let delta = target - current;
      if (delta > height / 2) delta -= height;
      if (delta < -height / 2) delta += height;
      current = mod(current + (Math.abs(delta) < 0.24 ? delta : delta * 0.16));
    ...
    const move = (event: PointerEvent) => {
      if (drag === event.pointerId) target = mod(startTarget - (event.clientY - startY) * 1.08);
    };
    const up = () => {
      drag = null;
    };
    const key = (event: KeyboardEvent) => {
      if (event.key === "ArrowDown" || event.key === "ArrowUp") {
        ...
        target = mod(target + (event.key === "ArrowDown" ? 180 : -180));
      }
      if (event.code === "Space") {
        event.preventDefault();
        paused = !paused;
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="group"
      aria-label={`Comments column ${index + 1}. Arrow keys scroll; Space pauses.`}
    >
      <div className="test-col-track">
        {[0, 1].map((loop) => (
          <div className="test-group" key={loop} aria-hidden={loop === 1 || undefined}>
            {Array.from({ length: copies }, (_, copy) =>
              comments.map((comment) => (
                <article
                  className="test-card"
                  key={`${copy}-${comment.documentId}`}
                ...
                >
                  ...
                  <div className="test-person">
                    ...
                    <div>
                      <p className="test-name">{comment.personName}</p>
                      <p className="test-role">{comment.location}</p>
                    </div>
                  </div>
                </article>
              )),
...
export function TestimonialColumns({ comments }: { comments: Comment[] }) {
  ...
  const columns = Array.from({ length: count }, (_, i) =>
    comments.slice(i * size, (i + 1) * size),
  ).filter((items) => items.length);
  return (
    <div className="test-columns">
      {columns.map((items, index) => (
        <Column key={`${count}-${index}`} comments={items} index={index} />
      ))}
    </div>
  );
}
