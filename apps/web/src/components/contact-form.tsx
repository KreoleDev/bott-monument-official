...
export function ContactForm({
  ...
  const options = (details.inquiryTypes || "")
    .split("\n")
    .map((s) => s.trim())
    .filter(Boolean);
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      form.reset();
      setFeedback(details.successMessage || "Thank you. Your inquiry has been received.");
    } catch {
      setFeedback("Could not save your message. Please try again.");
    } finally {
      ...
      <fieldset disabled={pending}>
        ...
        <div className="cm-pills">
          {options.map((option, i) => (
            <label className="cm-pill" key={option}>
              <input
                className="sr-only"
                type="radio"
                name="inquiryType"
        ...
        <label className="sr-only" htmlFor="cmName">
          Your name
        </label>
        <input
          name="name"
          ...
          autoComplete="name"
        />
        <label className="sr-only" htmlFor="cmEmail">
          Email address
        </label>
        <input
          name="email"
          ...
          autoComplete="email"
        />
        <label className="sr-only" htmlFor="cmMessage">
          Your message
        </label>
        <textarea
          name="message"
