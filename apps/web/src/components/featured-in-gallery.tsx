...
export function FeaturedInGallery({ section, covers }: { section: HomepageSection; covers: Cover[] }) {
  ...
  const thumbnail = (cover: Cover, index: number) => (
    <button key={cover.documentId} type="button" className={`ptw-cover${selected.documentId === cover.documentId ? " ptw-active" : ""}`}
  ...
  return <section id="press-clippings" aria-labelledby="featured-in-title"
    ...
    <div className="press-clippings-inner">
      <div className="press-clippings-header">
        ...
        <h2 id="featured-in-title" className="press-clippings-title">{section.title === "As Featured In" ? <>As <em>Featured</em> In</> : section.title}</h2>
      ...
      <div className="ptw-wrap">
        ...
        <div className="ptw-caption" aria-live="polite" aria-atomic="true">
          ...
          {selected.documentId !== initial.documentId && <button className="ptw-reset" type="button" onClick={() => setSelectedId(initial.documentId)}>Back to featured cover</button>}
        </div>
        <div className="ptw-side ptw-right">{sides.slice(middle).map((cover, index) => thumbnail(cover, middle + index))}</div>
      </div>
    </div>
  </section>;
}
