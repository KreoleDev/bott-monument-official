...
const config = ({ env }: Core.Config.Shared.ConfigParams): Core.Config.Plugin => ({
  ...
  ...(env('SMTP_HOST') ? {email: {config: {provider: 'nodemailer', providerOptions: {
    host: env('SMTP_HOST'), port: env.int('SMTP_PORT', 587), secure: env.bool('SMTP_SECURE', false),
  ...
  upload: {
    config: {
      ...(env('S3_BUCKET') ? {provider: 'aws-s3',providerOptions: {
        baseUrl: env('MEDIA_PUBLIC_URL'),
        s3Options: {region: env('S3_REGION'), endpoint: env('S3_ENDPOINT'), forcePathStyle: env.bool('S3_FORCE_PATH_STYLE', false),
