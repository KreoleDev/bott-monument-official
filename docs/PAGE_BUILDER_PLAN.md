...
Homepage rendering uses a small motor: registry → mapper → `usePage` →
...
1. `usePage` reads `Page.content` instead of a flat `Homepage Section` list.
...
- After the schema lands, tell the frontend: `usePage` will switch to `Page`
